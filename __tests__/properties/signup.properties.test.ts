...
describe("Feature: multi-step-signup - Property-Based Tests", () => {
  ...
  describe("Property 4: Form Validity Controls Button State", () => {
    it("should disable Continue when any required field is empty", () => {
      const emptyFieldGen = fc
        .record({
          full_name: fc.constantFrom("", "John Doe"),
    ...
    it("should enable Continue when all fields are valid", () => {
      ...
      fc.assert(
        fc.property(validFormGen, (formData) => {
          ...
          expect(formData.full_name.trim().length).toBeGreaterThanOrEqual(2);
      ...
      fc.assert(
        fc.property(
          ...
          (city, country) => {
            ...
            const withPhone = { city, country, phone_number: "123-456-7890" };
  ...
  describe("Property 11: Back Navigation Preserves Data", () => {
    it("should preserve form data structure through navigation", () => {
      const formDataGen = fc.record({
        role: fc.constantFrom("job_seeker", "employer", null),
        full_name: fc.string(),
        email: fc.string(),
        city: fc.string(),
        country: fc.string(),
      ...
      fc.assert(
        fc.property(formDataGen, (originalData) => {
          ...
          expect(deserialized.full_name).toBe(originalData.full_name);
    ...
    it("should maintain data integrity when navigating between steps", () => {
      fc.assert(
        fc.property(
          ...
          fc.integer({ min: 1, max: 6 }),
          fc.record({
            role: fc.constantFrom("job_seeker", "employer"),
            full_name: fc.string({ minLength: 1 }),
          }),
          (_fromStep, toStep, formData) => {
